 ...
 * e trava em HORA_FECHAMENTO (23h00 = 1380 min).
 *
 * WRAP DIÁRIO (decisão documentada, simplificação permitida pela missão):
 * quando o relógio atinge 1380 (23h), o dia reinicia IMEDIATAMENTE em 420
 * (7h00), sem esperar todos saírem — os personagens são reposicionados nos
 * pontos de spawn/postos (resetDia), os pincéis são recarregados
 * (resetPinceis: cargas a 100 % e descartados zerados, estoque preservado)
 * e o ciclo CHEGADA recomeça. Como `tickClock` só AVANÇA o relógio
 * (contrato), o retrocesso 1380→420 é feito com `useSchoolStore.setState`
 * direto + emissão manual do evento 'periodo'.
 */
...
/** Reinicia o dia: 23h → 7h, todos de volta aos pontos de origem. */
function wrapDia(m: Mundo): void {
  ...
  useSchoolStore.setState({
    ...
    turno: turnoPara(CONST.HORA_ABERTURA), // badge do HUD (W7) volta p/ 'manha'
  });
